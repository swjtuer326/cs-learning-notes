...
> **注意**: 单颗容量单位为 **Gb (Gigabit, 吉比特)**，不是 GB。1 GB = 8 Gb。DDR5 单颗最大容量 64Gb 为 JEDEC 标准定义的单 die 最大密度。
...
| LPDDR3  | 1.2V       | 智能手机、平板     |
| LPDDR4  | 1.1V       | 高端手机        |
| LPDDR4X | 0.6V (I/O), VDD=1.1V | 旗舰手机        |
...
> 信号完整性（反射、串扰、眼图等）的详细分析请参阅 [DDR性能优化与测量调试](06-DDR性能优化与测量调试.md) 的 10.2 节。
...
│                                                              │
│  1. 时序太复杂                                               │
│     ├── ACTIVATE → READ 之间要等 tRCD                        │
│     ├── READ 到数据返回要等 CL                                │
│     ├── 行切换要等 tRAS + tRP                                │
│     ├── 每 64ms 必须刷新一次 (REFRESH)                       │
│     └── CPU 哪有功夫管这些？                                  │
│                                                              │
│  2. 多个 Master 抢着用内存                                    │
│     ├── CPU 要读指令                                         │
...
│                                                              │
│  3. 地址映射不是直通的                                         │
│     ├── CPU 地址 0x80000000 ≠ DRAM 的 Row/Col/Bank           │
│     ├── 需要做地址重映射（interleaving 等）                    │
│     └── 不同 SoC 的映射方式完全不同                            │
│                                                              │
│  4. 带宽优化                                                 │
│     ├── 重新排序请求，减少行切换                               │
...
│ IP 厂商              │ 代表产品 / 说明                       │
├──────────────────────┼──────────────────────────────────────┤
│ Synopsys             │ DesignWare DDR Controller (uMCTL2)   │
│                      │ ★ 市场占有率最高，绝大多数 SoC 在用    │
├──────────────────────┼──────────────────────────────────────┤
│ Cadence              │ DDR PHY / Controller IP              │
│                      │ 高端 FPGA 和 ASIC 常见                │
├──────────────────────┼──────────────────────────────────────┤
│ ARM                  │ PL35x / PL341 系列                   │
│                      │ 用于早期的 ARM SoC                    │
...
│                      │ i.MX6/8/9 系列自研控制器              │
├──────────────────────┼──────────────────────────────────────┤
│ Rockchip             │ RK3399 / RK3588 DDR 控制器           │
│                      │ 自研 + Synopsys PHY 混合方案          │
├──────────────────────┼──────────────────────────────────────┤
│ Xilinx (AMD)         │ Zynq MPSoC DDR 控制器                │
│                      │ PS 端硬核控制器 + PL 端软控制器        │
├──────────────────────┼──────────────────────────────────────┤
│ RISC-V (平头哥等)    │ C920 DDR 控制器                      │
│                      │ 国产 RISC-V SoC 常用                  │
└──────────────────────┴──────────────────────────────────────┘

一句话: 如果你做嵌入式 Linux，大概率会遇到 Synopsys uMCTL2。
```
