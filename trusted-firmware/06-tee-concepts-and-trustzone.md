...
GIC (Generic Interrupt Controller, 通用中断控制器) 是 ARM 处理中断的标准 IP。在 TrustZone 安全扩展下,GIC 把所有中断分成两组:

| 中断组 | 信任级别 | 处理者 | 典型来源 |
|--------|----------|--------|----------|
| **Group 0** | 安全 | TEE OS(或 BL31) | 安全定时器、安全外设、FIQ 来源 |
| **Group 1** | 非安全 | Linux 内核 | 普通外设(网卡、USB、显卡) |

每个中断源(由中断号标识)在 GIC 的 GICD_IGROUPRn 寄存器中配置属于 Group 0 还是 Group 1。**配置由安全世界完成**——BL31/TEE OS 在启动时设置好,REE 无法修改。

**为什么要把中断分组?** 考虑一个安全加密引擎:它的中断必须送到 TEE,而不是 Linux,否则 Linux 可能伪造"加密完成"信号欺骗 TEE。把加密引擎中断配置为 Group 0,GIC 物理上保证这个中断只能被安全世界的 CPU 处理。

### 5.2 FIQ vs IRQ:中断路由

ARM 处理器有两种中断异常:**IRQ** 和 **FIQ**。在 TrustZone 下,GIC 用它们来区分中断组:

| CPU 当前状态 | Group 0 中断到达时 | Group 1 中断到达时 |
|--------------|-------------------|--------------------|
| 在非安全世界(Linux) | 触发 **FIQ**(被路由到 EL3 / S-EL1) | 触发 IRQ(由 Linux 处理) |
| 在安全世界(TEE) | 触发 IRQ(由 TEE OS 处理) | 触发 **FIQ**(被路由回非安全世界) |

**反直觉但合理**:Group 0 是安全中断,但当 CPU 在非安全世界时,它以 FIQ 形式到达——Linux 默认不处理 FIQ,FIQ 会被路由到 EL3 的 BL31,BL31 再切换到安全世界、把中断交给 TEE OS。
...
> **核心要点**:GIC 把中断按信任级别分成 Group 0(安全)/ Group 1(非安全),通过 FIQ/IRQ 路由保证"安全中断只能被安全世界处理"。这确保即使 Linux 被攻破,也无法伪造或截获 TEE 的中断。
