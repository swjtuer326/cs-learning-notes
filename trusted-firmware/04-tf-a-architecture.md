...
| `bl31_early_platform_setup()`  | BL31    | GIC、控制台初始化          | 是           |
...
通用 PSCI 库(`lib/psci/`)在收到 `PSCI_CPU_ON` 等 SMC 时,通过这个结构体调用平台实现。比如 `psci_cpu_on_start()` 调用 `psci_plat_pm_ops->pwr_domain_on()` 来物理上电目标 CPU——具体怎么上电(写电源控制器寄存器、通过 SCMI/mailbox 发消息给 SCP 等)由平台决定。
...
3. **证书**(6 个):TBBR 证书链——ROT Key Cert、Trusted Key Cert、4 个 Content Cert(BL2/BL31/BL32/BL33)
...
物理地址空间 (64-bit)
┌─────────────────────────────────────────┐
│ 0x0400_0000 - 0x0403_FFFF  (256 KB)    │
│ Trusted SRAM                           │
│ ├─ BL1 (ROM 副本)                      │
│ ├─ BL2 (验证加载阶段)                  │
│ └─ 安全数据区                          │
├─────────────────────────────────────────┤
│ 0x0600_0000 - 0x07FF_FFFF  (32 MB)     │
│ Trusted DRAM (TZC 保护)                │
│ ├─ BL31 (Secure Monitor)               │
│ ├─ BL32 (OP-TEE)                       │
│ └─ 安全堆栈                            │
...
| 机制                               | 保护对象                | 实现方式                          |
| -------------------------------- | ------------------- | ----------------------------- |
| **TZC (TrustZone Controller)**   | Trusted DRAM region | ARM TZC-400/TZC-500 硬件,配置安全/非安全访问权限 |
