...
> 上一章建立了三大主题的本质认知:Secure Boot 是启动时验证,TEE 是运行时隔离,TF-A 是 ARM 上的实现枢纽。一个自然的问题是:这些概念在 ARM 启动链中具体怎么体现?本章用启动链全景图回答——先看 ARM 的 5 阶段模型,再看各阶段职责与常驻逻辑。
...
> 上一章展示了 ARM 启动链的 5 阶段模型:BL1→BL2→BL31→BL32→BL33,信任从 ROM 逐级传递。一个自然的问题是:RISC-V 的启动链长什么样?它与 ARM 有什么本质差异?本章用 RISC-V 启动链全景图回答——先看启动流程,再对比两种架构的设计选择。
...
> 前两章分别展示了 ARM 和 RISC-V 的启动链全景。一个自然的问题是:Secure Boot 和 TEE 这两个主题之间是什么关系?它们是独立的还是可以分开实现?本章回答这个问题——先讲概念上的正交性,再讲工程上的耦合关系。
...
| 现代手机 | 有 | 有 | 完整安全栈 |
| 普通服务器 | 无 | 无 | 未启用 Secure Boot 的 x86 服务器(无 TEE 硬件扩展) |
...
> 上一章建立了 Secure Boot 与 TEE 的概念关系:Secure Boot 是基础,TEE 是应用,两者在工程上强耦合。一个自然的问题是:ARM 和 RISC-V 在实现这套安全模型时有什么本质差异?本章用对比表格回答——从隔离粒度、硬件机制、成熟度等维度系统对比两种架构。
...
> 前 5 章建立了三大主题的本质认知、ARM/RISC-V 启动链全景、概念关系与安全模型对比。本章为不同角色的读者提供学习路径建议,帮助你高效阅读后续 11 篇文档。
