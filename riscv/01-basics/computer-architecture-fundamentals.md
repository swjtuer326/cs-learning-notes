...
CPU 是计算机的"大脑"，它的核心职责是：**取指令 → 解码 → 执行**。让我们拆解它的内部结构：
...
| **异常（Exception）** | 指令执行 | 同步 | 除零错误、缺页、非法指令、系统调用 |
| **陷阱（Trap）** | 软件主动 | 同步 | `ecall` 系统调用、`ebreak` 断点 |

> RISC-V 规范中，"trap" 是中断和异常的统称，表示任何导致 CPU 改变正常执行流的事件。
