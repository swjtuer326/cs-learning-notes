...
| 阶段 | 产出 | 验证方式 |
|------|------|----------|
| ISA 基础 | 能读懂 RISC-V 汇编 | objdump 反汇编分析 |
| 特权架构 | 能写 trap 处理程序 | QEMU 运行裸机代码 |
| 裸机编程 | 能实现 UART 输出和定时器 | QEMU/开发板运行 |
| 虚拟内存 | 能建立页表并启用 MMU | 虚拟地址访问成功 |
| RTOS 移植 | 能在 RISC-V 上运行 RTOS | 多任务调度正常 |
| Linux 内核 | 能编写简单驱动 | insmod 加载运行 |
