...
你可以在 C 语言中写一辈子内核代码而不碰一行汇编——直到调试器停在一条 `ecall` 指令上，而你需要查看 `a0-a7` 的内容来理解系统调用参数是否正确；或者你写了一段关键路径的内联汇编实现自旋锁，却因为漏掉了 `"memory"` clobber 而导致编译器把 `load` 移到了 `store` 之前。在裸机固件开发中，你还得亲手写汇编入口：初始化栈指针、清零 BSS 段、跳转到 C 的 `main()`——这些操作没有标准库帮你完成，链接脚本也只负责放对地址，不负责初始化。

本章从 RISC-V 汇编基础结构讲起，覆盖伪指令展开、调用约定（寄存器角色、栈帧布局）、内联汇编的约束语法，最后用一个完整的裸机程序（汇编入口 + C 主函数 + 链接脚本 + Makefile + QEMU 运行命令）串起所有知识点。读完本章，你应当能用 `objdump -d` 自信地阅读任何 RISC-V ELF 文件的汇编输出。

> **工程师视角**：当你在内核中遇到无法解释的崩溃，或需要验证编译器是否正确生成原子操作时，`objdump -d` 输出的汇编就是你的第一现场。

## 学习目标

- 写出包含 `.text/.data/.bss` 段的完整 RISC-V 汇编程序
- 区分 `li/la/mv/call/ret` 等伪指令的真实指令展开
- 运用调用约定：了解 caller-saved 与 callee-saved 寄存器的分界线
- 绘制 RISC-V 栈帧布局图并解释 `sp` 16 字节对齐的原因
- 编写正确的 GCC 内联汇编，理解 `"=r"`、`"r"`、`"memory"` 约束
- 构建完整的裸机程序并在 QEMU 上运行调试
...
内联汇编让你在 C 中嵌入汇编片段，但真正的底层开发——固件、bootloader、嵌入式 OS 内核——需要你从零开始构建一个完整的裸机程序。下面是一个最小化的 RISC-V 裸机程序，包含汇编入口、C 主函数和链接脚本。
...
有了裸机骨架之后，系统软件工程师在日常工作中会遇到一些反复出现的需求——自旋锁、CSR 封装、延时函数、多核启动。下面是这些模式的惯用写法。
...
前面的代码模式都是手写汇编，但实际开发中大部分汇编来自编译器。理解编译器如何将 C 代码翻译成指令序列，能帮助你在性能分析和 bug 定位时更快地找到根因。
