...
| UR | Upstream Forwarding | 强制所有 P2P 上送 |
| DT | Direct Translated Enable | 允许 ATS Direct Translated P2P(详见 §1.4) |
| TU | Egress Control | 按 Requester ID 限制出口 |

对 P2P 影响最大的是 **RR / CR / UR** 三位。任意一位置位都会改变 P2P 路径:

- `RR=1`:EP 发给其他 EP 的请求被 Switch 上送,RC 再下发,绕道 RC
- `CR=1`:完成包同样绕道 RC
- `UR=1`:所有 P2P TLP 一律上送,完全禁止直传
...
| ACS RR/CR/UR | P2P 路径 | Switch 行为 | 性能 | 安全性 |
|--------------|---------|------------|------|--------|
| 0/0/0 | EP→Switch→EP(直传) | 转回下游 | 最优 | 弱(无 IOMMU 审计) |
| 0/0/1 | EP→Switch→RC→Switch→EP | UR 强制上送 | 最差(2 次穿越) | 强 |
| 1/0/0 | EP→Switch→RC→Switch→EP(请求绕) | 请求绕道,完成直传 | 中差 | 强 |
| 1/1/0 | EP→Switch→RC→Switch→EP(双向绕) | 请求与完成都绕道 | 差 | 强 |
| 1/1/1 | 完全绕道 | 等同 UR | 最差 | 最强 |

> **如何读这张表**:第一列是 ACS 控制位的组合,从左到右依次是 RR/CR/UR。**RR/CR/UR 任意一位为 1 都会改变 P2P 路径**——性能从最优(0/0/0)递减到最差(1/1/1),安全性反向递增。**实际系统**的 BIOS 默认设置通常是 `1/1/0`(请求与完成都 redirect),这是"安全优先"的取舍。要做 P2P 必须显式关闭这些位,常用内核参数 `pci=disable_acs_redir=<BDF>`。
...
`enum pci_p2pdma_map_type`(定义在 [include/linux/pci-p2pdma.h:31](file:///home/pbw/sg2046/linux-common/include/linux/pci-p2pdma.h#L31-L68))是 P2PDMA 框架的核心枚举,描述"客户端设备如何访问 provider 设备的 P2P 内存":
...
`calc_map_type_and_dist()`(定义在 [drivers/pci/p2pdma.c:686](file:///home/pbw/sg2046/linux-common/drivers/pci/p2pdma.c#L686-L779))是 P2PDMA 的核心决策函数。给定 provider 与 client,它要回答两个问题:能否 P2P?如果能,距离多远?其逻辑可拆为三段:
...
```c
/* 摘自 drivers/pci/p2pdma.c 第 531-554 行 pci_p2pdma_whitelist[] */
...
`pci_alloc_p2pmem()`(定义在 [drivers/pci/p2pdma.c:928](file:///home/pbw/sg2046/linux-common/drivers/pci/p2pdma.c#L928-L956))是驱动申请 P2P 内存的入口:
...
NTB 连接拓扑由 `enum ntb_topo`(定义在 [include/linux/ntb.h:78](file:///home/pbw/sg2046/linux-common/include/linux/ntb.h#L78-L86))描述:
...
`struct ntb_dev_ops`(定义在 [include/linux/ntb.h:261](file:///home/pbw/sg2046/linux-common/include/linux/ntb.h#L261-L334))是 NTB 硬件抽象的核心,包含约 40 个回调,分为五组:
...
# 触发 P2P 距离计算
cat /sys/bus/pci/devices/0000:02:00.0/p2pdma_map_type 2>/dev/null
# 或在 dmesg 查看警告
...
SG2046 是 Sophgo 的 RISC-V 服务器芯片,其 PCIe 控制器驱动 [drivers/pci/controller/dwc/pcie-sophgo.c](file:///home/pbw/sg2046/linux-common/drivers/pci/controller/dwc/pcie-sophgo.c) 仅实现 RC 模式:

```c
// 摘自 drivers/pci/controller/dwc/pcie-sophgo.c 第 1-4 行
...
驱动中只有 `sophgo_pcie_host_init()` 等 RC 端初始化函数,没有 `dw_pcie_ep_init()` 等 EP 初始化代码,也没有 NTB 相关 ops。这意味着 SG2046:
...
> **核心要点**:SG2046 选择 chiplet 而非 PCIe NTB 是合理的设计取舍——同 SoC 内多 die 通信对延迟极敏感(ns 级),PCIe NTB(μs 级)无法满足。NTB 的价值在跨 Host 场景,而 SG2046 的多 die 是单 Host 多 die,用 chiplet 更合适。这反映了"机制选型必须匹配物理拓扑"的工程原则——详见 [Controller 与 PHY 架构](./controller-phy-architecture.md) §6。
...
- 现有笔记:
  ...
  - [Controller 与 PHY 架构](./controller-phy-architecture.md) — §6 RK3588 vs SG2046
