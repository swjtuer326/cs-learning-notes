...
flowchart TD
    subgraph "上层 (虚拟机 / 用户态)"
        VM["VM / 用户态驱动<br/>QEMU · VFIO 应用"]
    end
    subgraph "本文研究对象"
        SRIOV["SR-IOV 虚拟化栈<br/>§1 规范 · §2 内核 · §3 ATS · §4 ACS · §5 VFIO"]
    end
    subgraph "下层 (硬件 / 系统服务)"
        PF["PF 物理功能<br/>控制面"]
        VF["VF 虚拟功能<br/>数据面"]
        IOMMU["IOMMU<br/>VT-d · SMMU · RISC-V IOMMU"]
    end

    VM -->|"VFIO ioctl 直通"| SRIOV
    SRIOV -->|"管理 VF 生命周期"| PF
    SRIOV -->|"分配 VF 给 VM"| VF
    VF -->|"DMA 经 IOMMU 隔离"| IOMMU
    PF -->|"PF-VF 邮箱配置"| VF
