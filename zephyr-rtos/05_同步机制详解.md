...
│   特性              信号量      互斥锁      条件变量     事件               │
│   ─────────────────────────────────────────────────────────────────────    │
│   互斥访问          支持        支持        不直接支持  不支持              │
│   优先级继承        不支持      支持        不支持      不支持              │
│   所有权            无          有          无          无                  │
│   递归锁定          不支持      支持        不适用      不适用              │
│   计数功能          支持        不支持      不支持      不支持              │
│   多条件等待        不支持      不支持      支持        支持                │
│   广播唤醒          不支持      不支持      支持        支持                │
│   ISR中使用         有限支持    不支持      不支持      支持                │
│   数据传递          不支持      不支持      不支持      不支持              │
│   内存占用          小          中          小          中                  │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
- [kernel/sem.c](file:///home/pbw/rtos/zephyr/kernel/sem.c) - 信号量实现
- [kernel/mutex.c](file:///home/pbw/rtos/zephyr/kernel/mutex.c) - 互斥锁实现
- [kernel/condvar.c](file:///home/pbw/rtos/zephyr/kernel/condvar.c) - 条件变量实现
- [kernel/events.c](file:///home/pbw/rtos/zephyr/kernel/events.c) - 事件实现
