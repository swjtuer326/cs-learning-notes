...
**2.** 条件变量 `k_condvar_wait()` 调用时，以下哪个操作是原子执行的？
...
**3.** 使用 `k_event_wait_all()` 等待多个事件时，以下哪种情况会返回？
...
**5.** 以下哪种同步机制最适合实现"等待多个条件之一满足"的场景？
...
**1.** 解释信号量的 P 操作（take）和 V 操作（give）的语义，以及它们如何实现线程同步。

**2.** 什么是虚假唤醒（Spurious Wakeup）？为什么条件变量等待需要使用 while 循环而不是 if 语句？

**3.** 描述优先级反转问题，并解释互斥锁的优先级继承机制如何缓解该问题。

**4.** 比较二进制信号量和互斥锁在保护临界区时的差异。

**5.** 解释事件机制中 `K_EVENT_WAIT_ANY` 和 `K_EVENT_WAIT_ALL` 的区别。

### 编程题

**1.** 使用互斥锁和条件变量实现一个线程安全的阻塞队列，支持 `put()` 和 `get()` 操作。

**2.** 实现一个读写锁，允许多个读者同时访问，但写者独占访问。

**3.** 使用事件机制实现一个简单的状态机，等待 START、STOP、PAUSE 三种事件。
...
**2.** 某系统使用信号量保护共享资源，但发现高优先级线程响应延迟增大。分析可能的原因和解决方案。
...
**选择题：** 1.B  2.A  3.D  4.A  5.D

**简答题答案要点：**

1. P 操作（take）：尝试获取信号量，若计数 > 0 则减 1 并继续，否则阻塞；V 操作（give）：增加计数，若有等待线程则唤醒一个。

2. 虚假唤醒是线程在没有被 signal 的情况下被唤醒。使用 while 循环可以确保唤醒后重新检查条件，避免条件不满足时错误执行。

3. 优先级反转：低优先级线程持有锁阻塞高优先级线程。优先级继承：临时提升持有者的优先级到等待者的优先级，使其尽快释放锁。

4. 二进制信号量：无所有权、无优先级继承、可在 ISR 中 give；互斥锁：有所有权、支持优先级继承、不能在 ISR 中使用。

5. `K_EVENT_WAIT_ANY`：任意一个指定事件发生即返回；`K_EVENT_WAIT_ALL`：所有指定事件都发生才返回。

**分析题答案要点：**

1. 死锁风险：两个线程以不同顺序获取锁，可能形成循环等待。解决方案：统一锁获取顺序，或使用 `k_mutex_lock` 带超时。

2. 原因：信号量不支持优先级继承，导致优先级反转。解决方案：使用互斥锁替代信号量保护临界区。

3. 问题：消费者没有在锁保护下检查条件，且没有使用 while 循环；消费者解锁操作可能出错（未先加锁）。正确做法是在锁保护下使用 while 循环检查条件。
