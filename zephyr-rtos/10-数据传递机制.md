...
> 第一章建立了"生产者-消费者需要传输管道"的直觉。但 Zephyr 提供了六种管道，它们之间到底有什么差异？本章直接引用 Zephyr 官方文档 `doc/kernel/services/index.rst` 第 56-65 行的对比矩阵，从六个维度横向比较，作为后续逐章剖析的导航。

### 2.1 官方对比表

下表翻译自 `file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/doc/kernel/services/index.rst` 第 56-65 行，是 Zephyr 官方对六种数据传递对象的高层特性对比：
...
`k_fifo` 实际上是 `k_queue` 的薄封装。`struct k_fifo` 的定义在 `file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/kernel.h` 第 2895-2906 行：
...
而 `struct k_queue`（同文件第 2255-2269 行）的内部数据结构是 `sys_sflist_t`（带标志位的单链表）：
...
官方 `fifos.rst` 还提到两个批量入队 API：`k_fifo_put_list(fifo, head, tail)` 把已串成单链表的一组数据项一次性追加到 FIFO；`k_fifo_put_slist(fifo, list)` 把 `sys_slist_t` 链表合并进 FIFO。批量入队的优势是**保证一组相关数据不被其他生产者的数据穿插**，且比逐个 `put` 更高效。官方示例（`fifos.rst` 第 99-119 行）展示了生产者线程用 `k_fifo_put` 发送 `struct data_item_t`，其首字段 `void *fifo_reserved` 即链表节点保留字：

```c
/* 来源：zephyr/doc/kernel/services/data_passing/fifos.rst 第 101-119 行 */
struct data_item_t {
...
如果用户数据无法预留首字（比如第三方库的结构体），可以用 `k_fifo_alloc_put`——内核会从调用线程的资源池分配一个 `struct alloc_node` 包装节点，把用户指针塞进去。源码见 `file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/queue.c` 第 25-28 行：
...
`k_fifo_put` 转发到 `k_queue_append`，最终调用 `queue_insert`（`file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/queue.c` 第 132-186 行）。核心逻辑分三步：
...
`k_fifo_get`（第 335-370 行）的对称逻辑：链表非空时取头节点返回；链表空且 `timeout != K_NO_WAIT` 时，调用 `z_pend_curr` 把当前线程挂到 `wait_q` 上阻塞。
...
`struct k_lifo` 的定义（`file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/kernel.h` 第 3146-3157 行）与 `k_fifo` 完全同构：
...
`struct k_stack` 的定义（`file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/kernel.h` 第 3284-3296 行）：
...
`k_stack_push`（`file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/stack.c` 第 101-136 行）：
...
`k_stack_pop`（第 148-190 行）是对称的退栈逻辑，栈空时按 `timeout` 阻塞或返回 `-EBUSY`。

> **关于 Stack 溢出行为**：官方对比矩阵（§2.1）将 Stack 溢出标记为"未定义行为"，官方 `stacks.rst` 第 36-40 行明确解释——当 `CONFIG_NO_RUNTIME_CHECKS` 启用时，内核**不会**检测并阻止向已满栈 push 数据，会导致数组越界和不可预测行为。但默认配置下 `kernel/stack.c` 的 `k_stack_push` 实现通过 `CHECKIF(stack->next == stack->top)` 检查并返回 `-ENOMEM`（见上方源码）。因此："未定义行为"是 `CONFIG_NO_RUNTIME_CHECKS` 开启时的规范行为，默认配置下实测返回 `-ENOMEM`。工程实践应假设最坏情况：不要依赖返回值，自己确保不溢出。
...
`struct k_msgq` 的定义（`file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/kernel.h` 第 5168-5204 行）：
...
`k_msgq_put` 的核心是 `put_msg_in_queue`（`file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/msg_q.c` 第 130-228 行）。关键逻辑：
...
官方 `message_queues.rst` 除了 `k_msgq_put` / `k_msgq_get` 外，还提供以下辅助 API（声明见 `file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/kernel.h` 第 5403-5474 行）：
...
官方 `message_queues.rst` 第 113-131 行示例展示了生产者用 `k_msgq_purge` 丢弃旧数据以保留新数据：

```c
/* 来源：zephyr/doc/kernel/services/data_passing/message_queues.rst 第 115-131 行 */
...
Mailbox 的核心是**消息描述符** `struct k_mbox_msg`（`file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/kernel.h` 第 5491-5514 行）：
...
发送方和接收方各填一个描述符，Mailbox 用 `mbox_message_match`（`file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/mailbox.c` 第 113-147 行）匹配两者：
...
官方 `mailboxes.rst` 第 236-256 行还展示了"空消息"用法——当 `info` 字段（32 位）已足够承载信息时，`size` 设为 0、`tx_data` 设为 `NULL`，省去数据缓冲：

```c
/* 来源：zephyr/doc/kernel/services/data_passing/mailboxes.rst 第 238-256 行 */
void producer_thread(void)
{
...
异步发送的机制（`file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/mailbox.c` 第 28-49 行）：
...
`struct k_pipe` 的定义（`file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/kernel.h` 第 5676-5695 行）：
...
**直传优化**：当读者已在等待时，写者的数据**绕过环形缓冲区**，直接 `memcpy` 到读者的缓冲区（`copy_to_pending_readers`，`file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/pipe.c` 第 93-145 行）。这在无环形缓冲（`buffer_size = 0`）的纯直传模式下尤其高效——数据从不落盘到中间缓冲。
...
`k_pipe_write` 的核心循环（`file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/pipe.c` 第 147-218 行）：
...
- [Zephyr Data Passing 官方文档](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/doc/kernel/services/index.rst) — 数据传递总览与对比矩阵（第 56-65 行）
