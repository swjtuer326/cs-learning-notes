...
**1.** 关于 Zephyr 内存管理，以下说法正确的是：

A. 堆分配是默认启用的
B. 静态分配具有确定性，适合实时系统
C. 内存池会产生内存碎片
D. 内存片（Memory Slab）支持变长分配

**2.** 以下哪个宏用于静态定义线程栈？

A. `K_THREAD_DEFINE()`
B. `K_THREAD_STACK_DEFINE()`
C. `K_STACK_DEFINE()`
D. `K_MEM_SLAB_DEFINE()`

**3.** 内存池（Memory Pool）与内存片（Memory Slab）的主要区别是：

A. 内存池支持多种大小的块，内存片只支持固定大小
B. 内存片更快，内存池更慢
C. 内存池不支持超时等待
D. 内存片会产生碎片

**4.** 关于内存域（Memory Domain），以下说法错误的是：
...
**1.** 比较静态内存分配、堆分配、内存池三种方式的优缺点和适用场景。

**2.** 解释内存池的块大小级别（Level）是如何工作的。

**3.** 什么是优先级反转？互斥锁如何通过优先级继承解决这个问题？

**4.** 描述 MPU（内存保护单元）在 Zephyr 内存保护中的作用。

**5.** 如何检测和避免内存泄漏？

### 编程题

**1.** 编写代码实现一个自定义内存池，支持 32、64、128 字节三种块大小，每种大小有 8 个块。

**2.** 实现一个线程安全的内存分配器，使用内存片管理固定大小的数据结构。

**3.** 编写代码创建一个内存域，将两个线程加入该域，并验证它们之间的内存隔离。
...
{
    buf = k_realloc(buf, 512);  // 扩展缓冲区
    // ... 处理数据
}
```

**2.** 某系统频繁分配和释放不同大小的内存块，运行一段时间后系统变慢。分析可能的原因和解决方案。

### 参考答案

**选择题：** 1.B  2.B  3.A  4.C  5.B

**简答题答案要点：**

1. 静态分配：确定性、无碎片，但不灵活；堆分配：灵活，但有碎片和非确定性；内存池：确定性、无碎片，但只支持固定大小块。

2. 内存池按 2 的幂次方划分块大小，请求时分配能容纳的最小级别，释放时自动合并。

3. 优先级反转是低优先级线程持有锁阻塞高优先级线程的现象。互斥锁通过优先级继承临时提升持有者优先级来解决。

4. MPU 提供硬件级别的内存保护，防止非法访问、栈溢出检测、用户/内核空间隔离。

5. 启用 `CONFIG_SYS_HEAP_RUNTIME_STATS` 监控堆使用；定期检查分配/释放配对；使用静态分析工具。

**分析题答案要点：**

1. 问题：`k_realloc` 可能返回新地址，但 `buf` 是局部变量，调用者持有的指针仍指向旧地址（可能已释放）。应返回新指针。

2. 可能是内存碎片问题。解决方案：使用内存池替代堆；定期整理内存；限制动态分配；预分配常用大小。
***
