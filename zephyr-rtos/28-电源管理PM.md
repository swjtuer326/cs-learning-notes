...
定义见 [include/zephyr/pm/state.h L28-L110](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/pm/state.h#L28)。
...
每个 PM 状态由 `struct pm_state_info` 描述（[include/zephyr/pm/state.h L115-L168](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/pm/state.h#L115)）：

```c
/* include/zephyr/pm/state.h L115-L168（节选） */
...
[state.c L14-L29](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/pm/state.c#L14) 用宏 `DT_FOREACH_CHILD_STATUS_OKAY` 把每个 CPU 引用的状态展开成静态数组：

```c
/* subsys/pm/state.c L14-L29 */
...
[state.c L50-L81](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/pm/state.c#L50) 提供两个查询函数：
...
policy 唯一对外暴露的入口是 `pm_policy_next_state(cpu, ticks)`（[policy_default.c L12](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/pm/policy/policy_default.c#L12)）：
...
[policy_default.c L12-L49](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/pm/policy/policy_default.c#L12) 的核心逻辑只有 30 行：

```c
/* subsys/pm/policy/policy_default.c L12-L49（节选） */
const struct pm_state_info *pm_policy_next_state(uint8_t cpu, int32_t ticks)
{
...
[state_lock.c L66-L82](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/pm/policy/policy_state_lock.c#L66) 实现了简单的引用计数锁：

```c
/* subsys/pm/policy/policy_state_lock.c L66-L82 */
...
`pm_policy_state_is_available()`（[state_lock.c L134-L151](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/pm/policy/policy_state_lock.c#L134)）的检查顺序是：先看 `global_lock_cnt != 0`（全局锁），再看 `lock_cnt[i] == 0`（逐状态锁），最后看 `latency_mask & BIT(i)`（延迟约束）——三者都通过才返回 true。`unlock_mask` 则由 `pm_policy_state_any_active()`（[L153-L163](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/pm/policy/policy_state_lock.c#L153)）使用，通过 `unlock_mask & latency_mask` 一次性判断"是否还有任何可用状态"。
...
[policy_latency.c L14-L23](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/pm/policy/policy_latency.c#L14) 维护一个全局最大延迟：

```c
/* subsys/pm/policy/policy_latency.c L14-L23 */
...
`update_max_latency()`（[policy_latency.c L26-L55](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/pm/policy/policy_latency.c#L26)）遍历所有请求，取**最小值**作为当前 `max_latency_us`——多个模块同时约束时，最严的胜出。
...
[state_lock.c L165-L176](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/pm/policy/policy_state_lock.c#L165) 在系统初始化时订阅 latency 变化：

```c
/* subsys/pm/policy/policy_state_lock.c L165-L176 */
...
[pm.c L173-L174](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/pm/pm.c#L173) 在 `pm_system_suspend` 入口先取 `events_ticks`：

```c
/* subsys/pm/pm.c L173-L174 */
events_ticks = pm_policy_next_event_ticks();
ticks = ticks_expiring_sooner(kernel_ticks, events_ticks);
```

`ticks_expiring_sooner`（[pm.c L77-L98](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/pm/pm.c#L77)）取两个 tick 数的较小者，把 `K_TICKS_FOREVER` 当作"无穷大"处理。
...
[include/zephyr/pm/device.h L68-L172](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/pm/device.h#L68) 定义了 4 个设备状态与 4 个动作：
...
[device.c L15-L26](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/pm/device.c#L15) 用两个静态表把 action 映射到"目标状态"和"期望的前置状态"：

```c
/* subsys/pm/device.c L15-L26 */
...
[device.c L48-L103](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/pm/device.c#L48) 是设备 PM 的核心入口：

```c
/* subsys/pm/device.c L48-L103（节选） */
...
[device.c L249-L302](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/pm/device.c#L249) 提供两类辅助 API：
...
`device_system_managed` 挂设备时会跳过 busy 和 wakeup-enabled 的设备（[device_system_managed.c L43-L45](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/pm/device_system_managed.c#L43)）。
...
[device_runtime.c L194-L309](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/pm/device_runtime.c#L194) 是 `pm_device_runtime_get` 的核心逻辑。关键步骤：
...
```c
/* subsys/pm/device_runtime.c L98-L113（节选） */
...
`PM_DEVICE_ISR_SAFE` 标志（[device.h L57-L65](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/pm/device.h#L57)）表示设备的 suspend/resume 很短，可在自旋锁临界区里完成。这种模式下：
...
[device_system_managed.c L27-L68](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/pm/device_system_managed.c#L27) 在 `pm_system_suspend` 决定进某个状态后，由 [pm.c L197-L211](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/pm/pm.c#L197) 调用：

```c
/* subsys/pm/pm.c L197-L211 */
...
[device_system_managed.c L36-L65](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/pm/device_system_managed.c#L36) 反向遍历设备数组：

```c
/* subsys/pm/device_system_managed.c L36-L65（节选） */
...
> **核心要点**：挂设备按"反初始化顺序"——后初始化的设备先挂。这与 [13 章 设备驱动模型](./13-设备驱动模型.md) 讲的 devicetree `ord` 顺序对应：子设备依赖父设备，初始化时父先子后，挂起时必须反过来先子后父，否则父设备掉电时子设备还在用。`pm_resume_devices`（[L70-L83](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/pm/device_system_managed.c#L70)）按相反顺序（先挂的最后 resume）恢复。
...
[device_system_managed.c L43-L45](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/pm/device_system_managed.c#L43) 显式跳过 `pm_device_runtime_is_enabled` 的设备——避免双重管理。一个设备要么归系统管，要么归运行时 PM 管，不能两边都管。
...
[pm_stats.c L15-L25](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/pm/pm_stats.c#L15) 用 Zephyr 的 stats 子系统为每个 CPU 的每个状态定义三个计数器：

```c
/* subsys/pm/pm_stats.c L15-L25 */
...
[pm.c L237-L251](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/pm/pm.c#L237) 在 `pm_state_set` 前后调用 `pm_stats_start/stop`：

```c
/* subsys/pm/pm.c L237-L251 */
...
`pm_stats_update`（[pm_stats.c L64-L72](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/pm/pm_stats.c#L64)）算出 `time_total = time_stop - time_start`，更新三个计数器：

```c
/* subsys/pm/pm_stats.c L64-L72 */
...
[pm.c L46-L75](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/pm/pm.c#L46) 提供通知机制：应用注册 `struct pm_notifier`，在进/出状态时被回调。
...
整个 PM 的入口是 [kernel/idle.c L22-L95](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/idle.c#L22) 的 `idle()` 函数。它是个死循环，每次循环检查 PM：
...
[pm.c L30-L33](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/pm/pm.c#L30) 定义了一个三选一的换算宏：

```c
/* subsys/pm/pm.c L30-L33 */
...
- idle 线程在 [kernel/idle.c L71](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/idle.c#L71) 调 `pm_system_suspend(_kernel.idle)`
