...
Zephyr 的向量表定义在 [arch/arm/core/cortex_m/vector_table.S](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/arch/arm/core/cortex_m/vector_table.S) 第 32-44 行：
...
第 39 行的 `.word z_main_stack + CONFIG_MAIN_STACK_SIZE` 是初始 MSP——指向 `z_main_stack`（main 线程栈）的栈顶。第 41 行的 `.word z_arm_reset` 是复位入口地址，CPU 上电后从这里开始执行。
...
`z_arm_reset` 在 [arch/arm/core/cortex_m/reset.S](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/arch/arm/core/cortex_m/reset.S) 第 83-233 行。它的工作是给 C 代码准备一个最小可用的环境，主要步骤编号如下：

1. 设置 MSP 指向 `z_main_stack + CONFIG_MAIN_STACK_SIZE`（第 147-148 行），保证栈可用
2. 清 `z_sys_post_kernel` 标志（仅 `CONFIG_DEBUG_THREAD_INFO` 时，第 155-160 行）
3. 调用 `soc_reset_hook`（若启用，第 162-164 行）—— SoC 厂商的自定义复位代码
4. 禁用中断：`cpsid i` 或设置 `BASEPRI`（第 179-187 行），保证初始化期间不被打断
5. 调用 `z_arm_watchdog_init`（若 `CONFIG_WDOG_INIT`，第 189-192 行）
6. 可选清栈：把 `z_interrupt_stacks` 填 `0xaa`（`CONFIG_INIT_STACKS`，第 202-207 行）
7. 设置 PSP 指向 `z_interrupt_stacks` 栈顶，切换到 PSP 运行（第 213-226 行）
8. 跳转到 `z_prep_c`（第 233 行）
...
`z_prep_c` 在 [arch/arm/core/cortex_m/prep_c.c](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/arch/arm/core/cortex_m/prep_c.c) 第 198-223 行。它做 5 件事：
...
`z_cstart` 在 [kernel/init.c](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/init.c) 第 538-610 行。主要步骤：

1. `gcov_static_init()`（第 541 行）—— 代码覆盖率钩子
2. `z_sys_init_run_level(INIT_LEVEL_EARLY)`（第 544 行）—— 跑 EARLY 级别所有 init 函数
3. `arch_kernel_init()`（第 547 行）—— 架构相关内核初始化
4. `LOG_CORE_INIT()`（第 549 行）—— 日志子系统核心初始化
5. `z_dummy_thread_init(&_thread_dummy)`（第 552 行，多线程时）—— 创建一个"假线程"作为当前上下文，让后续代码能引用 `k_current_get()`
6. `z_device_state_init()`（第 555 行）—— 初始化所有 `struct device` 的 state 字段
7. `soc_early_init_hook()` / `board_early_init_hook()`（第 557-559 行）
8. `z_sys_init_run_level(INIT_LEVEL_PRE_KERNEL_1)`（第 562 行）—— 跑 PRE_KERNEL_1 级别
9. `arch_smp_init()`（若 SMP，第 564 行）
10. `z_sys_init_run_level(INIT_LEVEL_PRE_KERNEL_2)`（第 566 行）—— 跑 PRE_KERNEL_2 级别
11. 初始化栈金丝雀（若 `CONFIG_REQUIRES_STACK_CANARIES`，第 568-574 行）
12. `timing_init()` / `timing_start()`（若启用，第 576-579 行）
13. `switch_to_main_thread(prepare_multithreading())`（第 582 行）—— 构造 main/idle 线程并切换过去，**不再返回**
...
`prepare_multithreading` 在 [kernel/init.c](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/init.c) 第 442-473 行：
...
`z_init_cpu(0)` 第 393-427 行做了三件事：调用 `init_idle_thread(0)` 创建 idle 线程、把 idle 线程指针存进 `_kernel.cpus[0].idle_thread`、把 `z_interrupt_stacks[0]` 栈顶存进 `_kernel.cpus[0].irq_stack`——这一步把 MSP 切换的目标准备好。

`switch_to_main_thread` 第 476-490 行最终调用 `z_swap_unlocked()` 触发上下文切换：保存当前 dummy 上下文、加载 main 线程的寄存器、跳到 `bg_thread_main`。从此代码运行在 main 线程上下文，栈是 `z_main_stack`，调度器已启动。

### 3.3 bg_thread_main：内核存活后的主流程

`bg_thread_main` 在 [kernel/init.c](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/init.c) 第 283-359 行。从这里开始，代码运行在真正的线程上下文，所有内核原语可用：

1. `z_mem_manage_init()`（MMU 启用时，第 295 行）
2. `z_sys_post_kernel = true`（第 297 行）—— 标记"内核已存活"，`k_is_pre_kernel()` 从此返回 false
3. `arch_irq_offload_init()`（若 `CONFIG_IRQ_OFFLOAD`，第 300 行）
4. `z_sys_init_run_level(INIT_LEVEL_POST_KERNEL)`（第 302 行）—— **跑 POST_KERNEL 级别**
5. `soc_late_init_hook()` / `board_late_init_hook()`（第 304-306 行）
6. `boot_banner()`（第 311 行）—— 打印启动 banner
7. `z_static_init_gnu()`（若 `CONFIG_STATIC_INIT_GNU`，第 314 行）—— 跑 GCC 风格的 `__init_array`
8. `z_sys_init_run_level(INIT_LEVEL_APPLICATION)`（第 318 行）—— **跑 APPLICATION 级别**
9. `z_init_static_threads()`（第 320 行）—— 启动所有 `K_THREAD_DEFINE` 定义的静态线程
10. `z_smp_init()` 与 `z_sys_init_run_level(INIT_LEVEL_SMP)`（SMP 启用时，第 327-330 行）—— 启动次核并跑 SMP 级别
11. `z_mem_manage_boot_finish()`（MMU 启用时，第 334 行）
12. 调用 `main()`（第 343 或 347 行）
13. `z_thread_essential_clear(&z_main_thread)`（第 351 行）—— main 退出后允许 main 线程被回收
...
下表来自 [include/zephyr/init.h](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/init.h) 第 20-46 行的官方注释与 [kernel/init.c](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/init.c) 第 126-135 行的 `enum init_level`：
...
```c
/* 来自 include/zephyr/init.h 第 150 行 */
#define SYS_INIT(init_fn, level, prio)
```

Zephyr 在 [kernel/init.c](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/init.c) 第 647-651 行就用 `SYS_INIT` 注册了两个对象核心列表的初始化：
...
- `z_sys_post_kernel` 全局变量：`bg_thread_main` 在跑 `POST_KERNEL` 之前置 true（[kernel/init.c](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/init.c) 第 297 行）
...
```c
/* include/zephyr/device.h 第 188 行 */
#define DEVICE_DEFINE(dev_id, name, init_fn, pm, data, config, level, prio, api)

/* include/zephyr/device.h 第 259 行 */
#define DEVICE_DT_DEFINE(node_id, init_fn, pm, data, config, level, prio, api, ...)

/* include/zephyr/init.h 第 150 行 */
#define SYS_INIT(init_fn, level, prio)
```
...
无论用哪个宏，最终都生成一个 `struct init_entry` 结构体放到链接脚本段里。结构体定义在 [include/zephyr/init.h](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/init.h) 第 66-77 行：
...
段名由 `Z_INIT_ENTRY_SECTION` 宏决定，定义在 [include/zephyr/init.h](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/init.h) 第 111-113 行：
...
链接脚本在 [include/zephyr/linker/common-rom/common-rom-kernel-devices.ld](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/linker/common-rom/common-rom-kernel-devices.ld) 第 5-21 行：
...
`CREATE_OBJ_LEVEL` 宏在 [include/zephyr/linker/linker-defs.h](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/linker/linker-defs.h) 第 70-75 行展开：
...
链接脚本末尾还有一个 `initlevel_error` 段（第 68-72 行）：
...
`z_sys_init_run_level` 在 [kernel/init.c](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/init.c) 第 220-250 行：
...
`do_device_init` 在 [kernel/device.c](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/device.c) 第 18-61 行：
...
注意第 50 行的 `dev->state->initialized = true`：**无论 init 成功还是失败都置 true**。这意味着失败的设备不会被自动重试。应用层可以通过 `device_is_ready(dev)` 检查设备是否就绪——它内部检查 `initialized && init_res == 0`。
...
带这个属性的设备，`DEVICE_DT_DEFINE` 会设置 `DEVICE_FLAG_INIT_DEFERRED` 标志。`z_sys_init_run_level` 看到这个标志就跳过自动初始化（[kernel/init.c](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/init.c) 第 242 行的 `if` 判断）。

应用稍后调用 `device_init(dev)` 显式初始化该设备，详见 [kernel/device.c](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/device.c) 第 63-70 行。`device_init` 内部检查 `dev->state->initialized`，若已初始化返回 `-EALREADY`，否则调用 `do_device_init`。
