...
> **关于 `_current` 和 `_current_cpu`**：
>
> - `_current`：宏，展开为 `_current_cpu->current`，即**当前 CPU 正在运行的线程**指针。之所以是宏而非全局变量，是因为 SMP 下每个 CPU 有各自的 `_current`。
> - `_current_cpu`：指向当前 CPU 的 `struct _cpu` 结构体的指针，这个结构体包含该 CPU 的 idle 线程、调度锁计数、swap_ok 标志、MetaIRQ 抢占信息等字段。单核时它指向唯一的 CPU 状态；SMP 时通过架构相关宏（如 `arch_curr_cpu()`）获取当前 CPU 的结构体地址。
...
> `next_up()` 的完整实现见[§7.0 调度核心](#70-调度核心-更新缓存与下一线程)。以下展示 SMP 特有的处理逻辑（线程挂起、亲和性检查等）：
...
**1.** 在 Zephyr 中，协作式线程的优先级范围是：

A. 0 到 CONFIG_NUM_COOP_PRIORITIES
B. -CONFIG_NUM_COOP_PRIORITIES 到 -1
C. 1 到 CONFIG_NUM_COOP_PRIORITIES
D. 任意正整数
...
**4.** 以下哪种就绪队列实现最适合线程数量多的场景？

A. SCHED_SIMPLE（简单链表）
B. SCHED_SCALABLE（红黑树）
C. SCHED_MULTIQ（多队列）
D. 以上都一样

**5.** 在 SMP 系统中，`k_thread_cpu_pin()` 的作用是：
...
**1.** 解释协作式调度和抢占式调度的区别，以及各自的适用场景。

**2.** 描述调度器锁定（`k_sched_lock`）的工作原理和使用注意事项。

**3.** 解释 EDF（最早截止时间优先）调度算法的工作原理。

**4.** 比较三种就绪队列实现（链表、红黑树、多队列）的优缺点。

**5.** 描述 SMP 系统中 IPI（处理器间中断）在调度中的作用。

### 编程题

**1.** 编写代码创建三个线程，分别使用协作式、抢占式和 MetaIRQ 优先级，观察它们的执行顺序。

**2.** 实现一个自定义的时间片轮转测试，验证同优先级线程的轮流执行。

**3.** 在 SMP 系统中，编写代码将特定线程绑定到 CPU 0，并验证其不会迁移到其他 CPU。
...
- CONFIG_TIMESLICE_SIZE = 10ms

线程：
- Thread A: 优先级 K_PRIO_COOP(0)
- Thread B: 优先级 K_PRIO_PREEMPT(5)
- Thread C: 优先级 K_PRIO_PREEMPT(5)
- Thread D: 优先级 K_PRIO_PREEMPT(10)

描述各线程的执行顺序和时间分配。
```

**2.** 某实时系统要求任务必须在截止时间前完成，但使用固定优先级调度时出现任务错过截止时间的情况。分析可能的原因和解决方案。

**3.** 分析以下代码在 SMP 系统中的潜在问题：
...
// 创建两个相同优先级的线程运行 increment_thread
```
...
1. 协作式：线程主动让出 CPU，不会被抢占，适合实时性要求高、执行时间可预测的任务；抢占式：高优先级线程可抢占低优先级线程，适合响应速度要求高的场景。

2. `k_sched_lock()` 增加锁计数，阻止调度器进行抢占式调度；`k_sched_unlock()` 减少锁计数，计数为 0 时恢复调度。注意：锁定期间不要调用阻塞操作。

3. EDF 根据任务的截止时间动态调整优先级，截止时间最近的任务优先级最高。适用于周期性实时任务。

4. 链表：内存小，但查找 O(n)；红黑树：查找 O(log n)，适合线程多；多队列：O(1) 操作，但内存占用较大。

5. IPI 用于通知其他 CPU 进行调度决策，如唤醒高优先级线程、负载均衡等。

**分析题答案要点：**

1. Thread A（协作式优先级最高）先执行，直到主动让出；Thread B 和 C（同优先级抢占式）按时间片轮转；Thread D（最低优先级）在 B、C 都阻塞时执行。

2. 可能原因：优先级设置不合理、优先级反转、任务执行时间超过预期。解决方案：使用 EDF 调度、调整优先级、使用优先级天花板协议。

3. 问题：`shared_counter++` 不是原子操作，在 SMP 系统中可能产生竞态条件。解决方案：使用互斥锁保护、使用原子操作 `atomic_inc()`。
