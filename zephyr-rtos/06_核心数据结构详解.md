...
**1.** `struct k_thread` 中的 `callee_saved` 字段的作用是：
...
**2.** `_thread_base` 结构中使用联合体存储 `prio` 和 `sched_locked` 的目的是：
...
**3.** `struct _timeout` 中的 `dticks` 字段存储的是：
...
**1.** 解释 `struct k_thread` 与 `struct _thread_base` 的关系，为什么要分离设计？

**2.** 描述线程状态标志 `_THREAD_PENDING`、`_THREAD_SLEEPING`、`_THREAD_QUEUED` 的含义和区别。

**3.** 解释 `k_timeout_t` 类型的设计，为什么使用结构体而不是整数？

**4.** 描述 `struct k_mutex` 如何实现优先级继承。

**5.** 比较三种就绪队列实现（链表、红黑树、多队列）的时间复杂度。

### 编程题

**1.** 编写代码遍历系统中所有线程，打印每个线程的名称、优先级和状态。

**2.** 实现一个自定义的定时器链表，支持添加、删除和触发超时回调。

**3.** 编写代码检测线程栈使用情况，并在栈溢出时打印警告。

### 分析题

**1.** 分析以下代码中 `preempt` 字段的判断逻辑：
...
为什么协作式线程（优先级为负数）的 `preempt` 值会大于 `_PREEMPT_THRESHOLD`？

**2.** 某系统在 SMP 模式下运行，发现线程在 CPU 间频繁迁移导致性能下降。分析可能的原因和解决方案。

**3.** 分析超时队列使用增量时间（而非绝对时间）存储的设计优势。

### 参考答案

**选择题：** 1.B  2.B  3.B  4.D  5.C

**简答题答案要点：**

1. `k_thread` 包含完整线程信息，`_thread_base` 只包含调度核心信息。分离设计使调度器只需操作核心信息，提高效率和代码复用。

2. `_THREAD_PENDING`：等待同步对象；`_THREAD_SLEEPING`：主动睡眠；`_THREAD_QUEUED`：在就绪队列中。

3. 使用结构体提供类型安全，支持多种时间单位（tick、毫秒、秒等），并区分 `K_NO_WAIT` 和 `K_FOREVER`。

4. 当高优先级线程等待互斥锁时，临时提升持有者的优先级到等待者的优先级，释放后恢复原始优先级。

5. 链表：插入 O(n)、删除 O(1)；红黑树：插入删除 O(log n)；多队列：插入删除 O(1)。

**分析题答案要点：**

1. 协作式线程优先级为负数（如 -1），存储在 `int8_t prio` 中。由于小端序，`preempt`（uint16_t）的高字节为 `sched_locked`，低字节为负数优先级，整体值较大。

2. 原因：线程未设置 CPU 亲和性，调度器负载均衡策略。解决方案：使用 `k_thread_cpu_pin()` 绑定线程到特定 CPU。

3. 增量时间只需更新队首节点的值，减少时钟中断处理开销；绝对时间需要遍历更新所有节点。
