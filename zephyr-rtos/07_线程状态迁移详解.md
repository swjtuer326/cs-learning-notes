...
> **阅读指南**：这张图分为四个区域——
> - **稳态区（上）**：_READY、_RUNNING、_PENDING、_DEAD——线程在这些状态中"停留"
> - **过渡态区（中）**：_QUEUED、_PRESTART、_DORMANT——状态切换时的中间态
> - **等待区（下）**：_WAITING——等待条件的展开视图
> - **特殊态（右）**：`execution_state` 辅助字段，独立于主状态编码
>
> 箭头上的标签是**触发操作**（函数调用或事件），线的方向是**状态转换方向**。
...
**1.** 线程调用 `k_sem_take()` 获取信号量失败时，线程会进入什么状态？
...
**4.** 调用 `k_thread_suspend()` 后，线程状态会发生什么变化？

A. 设置 `_THREAD_SUSPENDED`，清除 `_THREAD_QUEUED`
B. 设置 `_THREAD_SUSPENDED`，保持 `_THREAD_QUEUED`
C. 清除 `_THREAD_QUEUED`，设置 `_THREAD_SLEEPING`
D. 设置 `_THREAD_DEAD`

**5.** 线程从 `_THREAD_PENDING` 状态恢复到就绪状态，需要满足什么条件？

A. 超时到期
B. 等待的资源可用
C. 调用 `k_thread_resume()`
D. B 或 C

### 简答题

**1.** 解释 `_THREAD_PENDING` 和 `_THREAD_SLEEPING` 状态的区别。

**2.** 描述线程从创建到首次运行的完整状态迁移过程。

**3.** 为什么线程状态使用位标志而不是枚举？

**4.** 解释 `z_is_thread_ready()` 函数的判断逻辑。

**5.** 线程终止时（`_THREAD_DEAD`），为什么需要唤醒等待该线程的其他线程？

### 编程题

**1.** 编写代码监控线程状态变化，打印线程进入和退出每个状态的时间戳。

**2.** 实现一个线程状态查询函数，返回线程状态的字符串描述。

**3.** 编写代码演示线程状态迁移：创建线程 → 挂起 → 恢复 → 睡眠 → 终止。
...
```
时刻 T1: 线程 A 调用 k_sem_take(&sem, K_MSEC(100))，信号量不可用
时刻 T2: 50ms 后，其他线程调用 k_sem_give(&sem)
时刻 T3: 线程 A 继续执行
```

**2.** 某线程在等待互斥锁时被 `k_thread_abort()` 终止，分析这对互斥锁的影响。
...
{
    k_thread_suspend(k_current_get());  // 挂起自己
    printk("Resumed\n");  // 这行代码会执行吗？
}
```
...
**选择题：** 1.B  2.C  3.B  4.A  5.D

**简答题答案要点：**

1. `_THREAD_PENDING`：等待同步对象（信号量、互斥锁等）；`_THREAD_SLEEPING`：主动睡眠（`k_sleep`）。

2. 创建 → `_THREAD_SLEEPING`（初始）→ `k_thread_start()` → `_THREAD_QUEUED`（就绪）→ 调度器选中 → 运行。

3. 位标志允许线程同时处于多个状态（如 `_PENDING | _QUEUED`），更灵活；枚举只能表示单一状态。

4. 判断线程状态中是否没有阻止运行的标志（`_PENDING`、`_SLEEPING`、`_DEAD`、`_DUMMY`、`_SUSPENDED`）。

5. 实现 `k_thread_join()` 功能，等待线程结束的其他线程需要被唤醒以继续执行。

**分析题答案要点：**

1. T1：`_THREAD_PENDING`；T2：信号量可用，线程状态变为 `_THREAD_QUEUED`；T3：线程被调度执行。

2. 线程终止后，互斥锁的等待队列会清理该线程，但互斥锁本身不会被释放，可能导致死锁。需要确保线程持有资源时不能被强制终止。

3. 会执行。`k_thread_suspend()` 挂起当前线程，但其他线程可以调用 `k_thread_resume()` 恢复它，恢复后继续执行。
