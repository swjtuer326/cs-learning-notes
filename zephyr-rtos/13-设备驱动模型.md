...
源码位于 [include/zephyr/device.h](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/device.h) 第 513-553 行：
...
`struct device_state` 源码位于 [include/zephyr/device.h](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/device.h) 第 458-472 行：
...
`device_is_ready()` 的实现就检查这两个字段（[kernel/device.c](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/device.c) 第 186-197 行）：
...
`struct device_ops` 源码位于 [include/zephyr/device.h](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/device.h) 第 500-508 行，目前包含 `init` 与可选 `deinit`：
...
`do_device_init()`（[kernel/device.c](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/device.c) 第 18-61 行）通过 `dev->ops.init` 调用驱动初始化函数，把返回码记入 `state->init_res`。
...
下表对照三个核心宏的参数与用途（源码见 [include/zephyr/device.h](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/device.h) 第 188-191、259-262、283-284 行）：
...
源码（[include/zephyr/device.h](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/device.h) 第 283-284 行）：
...
`DEVICE_DT_DEFINE` 内部调用 `Z_DEVICE_DEFINE`（[include/zephyr/device.h](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/device.h) 第 1317-1336 行），后者做四件事：
...
`struct init_entry` 源码（[include/zephyr/init.h](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/init.h) 第 66-77 行）：
...
`Z_INIT_ENTRY_SECTION(level, prio, sub_prio)` 把每个 `init_entry` 放入按级别、优先级、子优先级命名的链接段（[include/zephyr/init.h](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/init.h) 第 111-113 行）：
...
定义一个子系统 API 分两步。第一步在头文件声明 API 结构体，用 `__subsystem` 修饰（[include/zephyr/drivers/gpio.h](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/drivers/gpio.h) 第 810 行）：
...
第二步在驱动 C 文件中用 `DEVICE_API` 宏定义实例（[include/zephyr/device.h](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/device.h) 第 1365 行），如 [drivers/gpio/gpio_nrfx.c](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/drivers/gpio/gpio_nrfx.c) 第 650 行：
...
子系统头文件提供一组 `static inline` 包装函数，把"`dev->api` 取指针再调用"封装起来。`gpio_port_set_bits_raw` 的实现（[include/zephyr/drivers/gpio.h](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/drivers/gpio.h) 第 1410-1422 行）：
...
应用调用 `gpio_pin_set_dt(&led, 1)` 时的完整跳转链（参见 [include/zephyr/drivers/gpio.h](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/drivers/gpio.h) 第 1690-1706、1650-1667、1410-1422 行）：
...
Zephyr 把启动过程划分为 6 个有序级别（[include/zephyr/init.h](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/init.h) 第 20-46 行）：
...
驱动 init 时应检查所依赖的设备是否就绪。nRF GPIO 驱动的 init 函数（[drivers/gpio/gpio_nrfx.c](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/drivers/gpio/gpio_nrfx.c) 第 623-648 行）：
...
加了此属性的设备启动时**不会**自动调 init，`device_is_ready()` 返回 false。应用在使用前需调用 `device_init(dev)` 手动初始化（[include/zephyr/device.h](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/device.h) 第 906 行）：

```c
__syscall int device_init(const struct device *dev);
```

`device_init()` 内部调 `do_device_init()`（[kernel/device.c](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/device.c) 第 63-70 行），后者调用 `dev->ops.init(dev)` 完成实际初始化，并把结果记入 `state->init_res`。
...
此时 `DEVICE_MMIO_ROM` 可省略——地址不来自 DTS。`device_map()` 源码见 [include/zephyr/sys/device_mmio.h](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/sys/device_mmio.h) 第 96-100 行。
...
nRF GPIO 驱动的多实例代码（[drivers/gpio/gpio_nrfx.c](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/drivers/gpio/gpio_nrfx.c) 第 690-718 行）：
...
`SYS_INIT` 注册一个"无设备"的初始化函数，让它按级别/优先级在启动时被调用。源码（[include/zephyr/init.h](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/init.h) 第 150-151 行）：
...
`SYS_INIT_NAMED` 展开（[include/zephyr/init.h](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/init.h) 第 166-169 行）：
...
子系统 API 在头文件中用 `@since` 与 `@version` 标注版本（参见 [官方 API 设计指南](../zephyr-project/zephyr/doc/develop/api/design_guidelines.rst)）。例如 `device.h` 顶部（[include/zephyr/device.h](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/device.h) 第 32-34 行）：
