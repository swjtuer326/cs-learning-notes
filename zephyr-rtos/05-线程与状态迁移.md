...
```c
/* include/zephyr/kernel/thread.h L46-L134（节选） */
struct _thread_base {
    union {
...
```c
/* kernel/thread.c L617（节选自 z_setup_new_thread） */
...
`k_thread_start()` 现在是 `k_wakeup()` 的薄包装（见 [include/zephyr/kernel.h L1304](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/kernel.h#L1304)）：

```c
/* include/zephyr/kernel.h L1304-L1307 */
...
Zephyr 用 8 位标志字段表示状态，而非枚举。定义在 [include/zephyr/kernel_structs.h L52-L73](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/kernel_structs.h#L52)：

```c
/* include/zephyr/kernel_structs.h L52-L73 */
...
判断函数都是位与操作（[kernel/include/kthread.h L88-L143](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/include/kthread.h#L88)）：

```c
/* kernel/include/kthread.h L117-L133 */
...
状态对应的字符串（用于 Shell `kernel threads` 命令）定义在 [kernel/include/kthread.h L16-L23](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/include/kthread.h#L16)：

```c
/* kernel/include/kthread.h L16-L23 */
...
线程类别的判断在 [kernel/include/kthread.h L65-L81](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/include/kthread.h#L65)：

```c
/* kernel/include/kthread.h L65-L81 */
...
```c
/* include/zephyr/kernel_structs.h L164-L167 */
#if (CONFIG_NUM_METAIRQ_PRIORITIES > 0)
...
essential 标志的判断与设置在 [kernel/include/kthread.h L198-L221](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/include/kthread.h#L198)：

```c
/* kernel/include/kthread.h L198-L221 */
...
注意 `K_ESSENTIAL` 是**运行时可改**的——main 线程在 `bg_thread_main()` 完成初始化、调用 `main()` 返回后，会主动 `z_thread_essential_clear()` 清除自己的 essential 标志（见 [kernel/init.c L351](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/init.c#L351)），允许后续正常终止。

> **为什么 `K_USER` 用户线程不能创建 essential 线程？** 因为 essential 线程拥有"终止即 panic"的特权，等价于"系统级"权限。用户线程受限的本质就是"不能影响整个系统"——若允许它创建 essential 线程，它就能通过终止该线程触发 panic，相当于获得了 DoS 整个系统的能力。源码在 [kernel/thread.c L854-L855](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/thread.c#L854) 显式拒绝：`K_OOPS(K_SYSCALL_VERIFY(!(options & K_ESSENTIAL)))`。
...
main 线程是内核启动时自动创建的"背景线程"，其入口函数是 `bg_thread_main()`（见 [kernel/init.c L283-L359](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/init.c#L283)）。它完成的工作远超"调用 `main()`"：
...
main 线程的创建在 [kernel/init.c L459-L468](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/init.c#L459)：

```c
/* kernel/init.c L459-L468（节选） */
...
注意 `z_main_stack` 的定义（[kernel/init.c L55](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/init.c#L55)）是 `K_THREAD_PINNED_STACK_DEFINE(z_main_stack, CONFIG_MAIN_STACK_SIZE)`，属于 `K_THREAD_STACK` 家族而非 `K_KERNEL_STACK`。这是因为 main 线程未来可能需要切换到用户模式或与用户线程共享栈区，`K_THREAD_PINNED_STACK` 在满足 MPU 对齐约束的同时支持这种扩展，而 `K_KERNEL_STACK` 不支持用户模式。
...
idle 线程在每个 CPU 上都有一个，是"最后兜底"的线程。它的入口函数是 `idle()`（[kernel/include/kthread.h L37](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/include/kthread.h#L37)），核心是一个无限循环：检查是否有电源管理可做，否则进入低功耗等待中断。

idle 线程的创建在 [kernel/init.c L362-L391](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/kernel/init.c#L362)：

```c
/* kernel/init.c L362-L391（节选） */
...
`k_thread_runtime_stats_t` 的字段定义在 [include/zephyr/kernel/thread.h L207-L240](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/kernel/thread.h#L207)，主要包括：
