...
[11 章核心数据结构](./11-核心数据结构.md) §6-§8 已经介绍了 4 个并发队列的基本概念。本篇深入源码，把它们分成两条技术路线对照：
...
> **核心要点**：本篇与 [11 章核心数据结构](./11-核心数据结构.md) §6-§8 不重复——11 章只讲基本概念与 API 表，本篇深入到 4 索引状态机、busy 位覆盖路径、Vyukov 算法的 atomic swap 串链细节、跨核 cache 维护时序。
...
源码 [include/zephyr/sys/mpsc_pbuf.h:90-128](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/sys/mpsc_pbuf.h) 定义：
...
[11 章核心数据结构](./11-核心数据结构.md) §6.1 只列了 4 个索引的字段名与"两阶段提交"的一句话说明，本篇深入到 4 索引如何配合 `k_spinlock` 与 `k_sem` 完成正确的并发推进——这是日志后端在多 ISR 高频提交下不丢半填数据的根基。
...
源码 [lib/os/mpsc_pbuf.c:341-413](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/lib/os/mpsc_pbuf.c) 的 `mpsc_pbuf_alloc` 与 [lib/os/mpsc_pbuf.c:415-427](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/lib/os/mpsc_pbuf.c) 的 `mpsc_pbuf_commit`。生产者的两步：
...
源码 [lib/os/mpsc_pbuf.c:536-586](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/lib/os/mpsc_pbuf.c) 的 `mpsc_pbuf_claim` 与 [lib/os/mpsc_pbuf.c:588-620](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/lib/os/mpsc_pbuf.c) 的 `mpsc_pbuf_free`。消费者的两步：
...
- **`mpsc_pbuf_put_word(buf, item)`**——源码 [lib/os/mpsc_pbuf.c:295-339](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/lib/os/mpsc_pbuf.c)：单次取锁，原子地写一个字并推进 `tmp_wr_idx` 与 `wr_idx`
- **`mpsc_pbuf_put_word_ext(buf, item, data)`**——源码 [lib/os/mpsc_pbuf.c:429-483](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/lib/os/mpsc_pbuf.c)：写一个字 + 一个指针（共 2 字），同样原子提交
...
源码 [include/zephyr/sys/mpsc_packet.h:32-53](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/sys/mpsc_packet.h)：
...
**关键设计**：2 bit 内部 header 占用首字最高位，剩余 30 bit 由用户复用。这是与 [11 章](./11-核心数据结构.md) §2.4 `sys_sflist_t` 复用 next 指针低位同构的"无成本复用"思路——对齐约束让首字的高位本就空闲，借来存状态是零开销。
...
源码 [lib/os/mpsc_pbuf.c:118-147](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/lib/os/mpsc_pbuf.c)：
...
`flags` 中的 `MPSC_PBUF_MODE_OVERWRITE`（源码 [include/zephyr/sys/mpsc_pbuf.h:59](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/sys/mpsc_pbuf.h)）决定缓冲区满时的行为：
...
源码 [lib/os/mpsc_pbuf.c:175-264](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/lib/os/mpsc_pbuf.c) 是覆盖路径的核心。遇到 `rd_idx` 处的包时，按以下顺序处理：
...
busy 包分支的核心源码 [lib/os/mpsc_pbuf.c:207-231](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/lib/os/mpsc_pbuf.c)：
...
源码 [lib/os/mpsc_pbuf.c:588-620](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/lib/os/mpsc_pbuf.c) 的 `mpsc_pbuf_free`。消费者 free 一个 busy 包时，要检查它是否已被覆盖路径绕过：
...
源码 [include/zephyr/sys/spsc_pbuf.h:66-91](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/sys/spsc_pbuf.h)：
...
源码 [lib/os/spsc_pbuf.c:29-38](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/lib/os/spsc_pbuf.c) 注释解释了包格式设计：
...
- **2 字节大端存长度**——大端序让首字节是 MSB，长度必须严格小于 `SPSC_PBUF_MAX_LEN`（定义为 `0xFF00`，[include/zephyr/sys/spsc_pbuf.h:57](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/sys/spsc_pbuf.h)），即实际最大长度为 `0xFEFF`，让长度首字节永远 < `0xFF`
...
源码 [lib/os/spsc_pbuf.c:137-200](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/lib/os/spsc_pbuf.c) 的 `spsc_pbuf_alloc` 与 [lib/os/spsc_pbuf.c:202-227](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/lib/os/spsc_pbuf.c) 的 `spsc_pbuf_commit`。生产者两步：
...
源码 [lib/os/spsc_pbuf.c:250-318](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/lib/os/spsc_pbuf.c) 的 `spsc_pbuf_claim`。消费者两步：
...
源码 [include/zephyr/sys/mpsc_lockfree.h:79-90](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/sys/mpsc_lockfree.h)：
...
**关键设计**：`stub` 是哨兵节点，初始化时 `head = tail = &stub`。这让"空队列"和"非空队列"走同一段代码——永远没有 NULL 检查。这与 [11 章](./11-核心数据结构.md) §3.1 `sys_dlist` 用链表头本身当哨兵的思路同构。

### 7.2 单核 vs SMP 的指针抽象

源码 [include/zephyr/sys/mpsc_lockfree.h:43-74](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/sys/mpsc_lockfree.h) 在 `!CONFIG_SMP` 时把 `mpsc_ptr_t` 退化为普通指针 + volatile，SMP 时退化为 `atomic_ptr_t`：
...
**为什么单核不用原子指令？** 源码注释 [include/zephyr/sys/mpsc_lockfree.h:43-52](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/sys/mpsc_lockfree.h) 解释："On single core systems atomics are unnecessary and cause a lot of unnecessary cache invalidation. Using volatile to at least ensure memory is read/written by the compiler generated op codes is enough."——单核上原子指令的 cache 同步开销是纯浪费，volatile + 关中断就够了。

### 7.3 push：一次 atomic swap 串链

源码 [include/zephyr/sys/mpsc_lockfree.h:126-137](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/sys/mpsc_lockfree.h)：
...
源码 [include/zephyr/sys/mpsc_lockfree.h:145-187](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/sys/mpsc_lockfree.h)：
...
源码 [include/zephyr/sys/barrier.h:34-46](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/sys/barrier.h)：
...
源码 [lib/os/spsc_pbuf.c:202-227](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/lib/os/spsc_pbuf.c) 的 commit 路径，屏障顺序至关重要：
...
源码 [include/zephyr/sys/mpsc_lockfree.h:43-52](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/sys/mpsc_lockfree.h) 注释明确：
...
源码 [subsys/logging/log_core.c:12](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/logging/log_core.c) `#include <zephyr/sys/mpsc_pbuf.h>`，[subsys/logging/log_core.c:123](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/logging/log_core.c) 用 `STRUCT_SECTION_ITERABLE_ALTERNATE` 静态分配 `mpsc_pbuf_buffer`。
...
源码 [subsys/logging/log_core.c:127-138](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/subsys/logging/log_core.c)：
...
源码 [include/zephyr/rtio/rtio.h:108-112](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/rtio/rtio.h)：RTIO 用 `struct mpsc sq` 与 `struct mpsc cq` 分别承载提交队列与完成队列。
...
源码 [include/zephyr/rtio/sqe.h:396](file:///home/pbw/rtos/cs-learning-notes/zephyr-project/zephyr/include/zephyr/rtio/sqe.h) `struct rtio_iodev_sqe` 内嵌 `struct mpsc_node q`——这是侵入式无锁队列的体现。
...
> 本文从 [11 章核心数据结构](./11-核心数据结构.md) §6-§8 的基本概念深入到 Zephyr 源码实现细节。核心结论可以归纳为四点。
...
- [11-核心数据结构](./11-核心数据结构.md) §6-§8 — 基本概念与本篇的衔接
