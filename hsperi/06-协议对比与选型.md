...
**结论**：SPI NOR 启动 + eMMC 跑系统是嵌入式标配。SPI NOR 因为支持 XIP，CPU 可以直接执行启动代码而无需拷贝到 RAM；eMMC 容量大、速度快适合跑完整 OS。
...
Zephyr/Linux 都已支持 Octal SPI 控制器。
...
7. **06-协议对比与选型**（本篇）：建立横向对比能力，掌握选型决策
