...
### 4.1.1 五结构关系：软件抽象 ↔ 硬件实体

这五个结构体之间的关系可以浓缩为一张映射表：

| 结构体 | 对应硬件/实体 | 一句话 |
|--------|-------------|--------|
| `i2c_adapter` | **一条 I2C 总线控制器**（一个硬件 IP） | 比如 SoC 上的 `i2c0`、`i2c1`，每个 controller 一个 adapter |
| `i2c_algorithm` | **控制器与核心层的接口契约** | 核心层调 `algo->xfer()`，控制器驱动去填寄存器 |
| `i2c_client` | **总线上的一个从设备** | 比如挂在 `i2c0` 上的温度传感器 TMP102，地址 0x48 |
| `i2c_driver` | **从设备的驱动** | 负责 probe/bind、读写寄存器、上报数据 |
| `i2c_msg` | **一次传输片段**（地址 + 方向 + 数据） | 描述"往 0x48 写 1 字节 0x00"或"从 0x48 读 2 字节" |
...
flowchart LR
    subgraph "硬件世界"
        CTRL["I2C 控制器<br/>i2c0 / i2c1"]
        BUS["(总线)"]
        DEV["从设备<br/>TMP102 @0x48"]
        CTRL --- BUS --- DEV
    end
    subgraph "软件世界"
        ADAPTER["i2c_adapter<br/>控制器抽象"]
        ALGO["i2c_algorithm<br/>.xfer 回调"]
        CLIENT["i2c_client<br/>设备抽象"]
        DRIVER["i2c_driver<br/>设备驱动"]
        MSG["i2c_msg<br/>传输请求"]
    end
    ...
    DEV --> CLIENT
    CLIENT --- DRIVER
    DRIVER -.->|"调 i2c_transfer"| MSG
    MSG --> ADAPTER
```

> **如何读这张图**：`i2c_adapter` 是**控制器**的软件化身，`i2c_client` 是**从设备**的软件化身。两者通过总线物理连接。`i2c_algorithm` 描述"怎么往控制器塞命令"，`i2c_driver` 描述"收到数据后怎么处理"。`i2c_msg` 是这两端之间的**传输载荷**——从设备驱动构造它，交给 adapter 发出去。
