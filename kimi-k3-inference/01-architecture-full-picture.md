...
代价是 latent 表示有信息压缩,且路由路径拉长成 W↓ → 专家 FFN → W↑ 的近四次连续矩阵乘,条件数差、会放大内部激活。K3 在专家聚合与 W↑ 之间插 RMSNorm 把聚合表示幅值拉回固定范围,并用 SiTU-GLU 给 gate/up 两个分支的线性因子平滑封顶、压住离群点——这两项细节在 03 篇。
