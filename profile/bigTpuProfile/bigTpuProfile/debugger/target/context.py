...
class AKSFamilyContext(BModelContext):
    """Shared execution model for AKS-family chips.

    Concrete chips must provide their own target identity and register module.
    """
    ...
    def __init__(self) -> None:
        if self.device is None or self.regdef is None:
            raise TypeError("AKSFamilyContext cannot be instantiated directly")
        super().__init__()
    ...
    def merge_instruction(
        ...
        def fix_tgcr_cmd_id_dp(tiu_cmd: List[BaseTpuCmd]):
            for i, v in enumerate(tiu_cmd):
                if isinstance(v.reg, cls.regdef.SYS_TR_ACC_reg):
    ...
    def is_sys(cls, cmd: BaseTpuCmd):
        return isinstance(cmd.reg, (cls.dma_sys, cls.tiu_sys, cls.cdma_sys))

    def get_runner(self, memory_size: int) -> CModelRunner:
        assert self.using_cmodel, f"{self.device.name} currently only supports cmodel mode"
        if self._runner is None:
            self._runner = AKSRunner(memory_size, self.base_addr)
        return self._runner


class AKSContext(AKSFamilyContext):
    device = Target.AKS
    regdef = get_regdef("AKS")
    dma_sys = regdef.sDMA_sys_reg
    tiu_sys = regdef.SYS_reg
    cdma_sys = regdef.sCDMA_sys_reg


class AKSVContext(AKSFamilyContext):
    """AKSV execution context with its chip-specific register layout."""

    device = Target.AKSV
    regdef = get_regdef("AKSV")
    dma_sys = regdef.sDMA_sys_reg
    tiu_sys = regdef.SYS_reg
    cdma_sys = regdef.sCDMA_sys_reg
...
def get_context_cls(chip: str):
    try:
        return CONTEXT_BY_CHIP[chip.upper()]
    except (AttributeError, KeyError) as exc:
        supported = ", ".join(sorted(CONTEXT_BY_CHIP))
        raise ValueError(
