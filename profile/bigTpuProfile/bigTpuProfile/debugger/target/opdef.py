...
class TiuCmd(BaseTpuCmd, Tiu):
    opparam_converter = None  # assigned by AKSContext instance
...
class sfu_op(TiuCmd):
    ...
    def alg_cycle(self, alg_ops):
        dtype = self.operands[0].dtype
        return DIV_UP(alg_ops, info.EU_NUM(dtype) * info.NPU_NUM)
...
class sg_op(TiuCmd):
    ...
    eu_type = {
        ...
        # 2: "sg.pl_gather_rec", # not sure if is currently supported, not writen (Y) in AKS xlsx
