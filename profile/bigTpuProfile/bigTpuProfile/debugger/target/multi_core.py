...
def is_tiu_sys(reg):
    return reg.OP_NAME == "SYS"


def is_dma_sys(reg):
    return reg.OP_NAME == "sDMA_sys"


def is_sys(reg):
    return is_tiu_sys(reg) or is_dma_sys(reg)
    ...
    def __str__(self):
        ...
        if self.sys_cmds:
            ...
            if self.msgcore_id == 0:
                ...
                msg_result = []
                if is_tiu_sys(self.sys_cmds[1].reg):
                    msg_result = (
                        f"%B{self.sys_cmds[1].cmd_id}C{self.sys_cmds[1].core_id}"
                    )
                elif is_dma_sys(self.sys_cmds[1].reg):
                    msg_result = (
...
class MultiCore(Node):
    def __init__(self, core_id, core_nums, mlir_cmds: List[BaseTpuCmd], indent=0):
        ...
        self.msges: List[Msg] = [Msg()] * 512  # AKS has 512 * 14 bits msg que
