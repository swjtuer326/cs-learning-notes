...
if TYPE_CHECKING:
    from .context import AKSFamilyContext
...
class Decoder(DecoderBase):
    ...
    def __init__(self, context: "AKSFamilyContext") -> None:
        super().__init__()
        self.context = context
        self.op_class_dic = context.regdef.op_class_dic
        self.dma_sys = context.regdef.sDMA_sys_reg
        self.tiu_sys = context.regdef.SYS_reg
        self.cdma_sys = context.regdef.sCDMA_sys_reg

    def decode_tiu_cmd(
        self, reg_buf: memoryview, *, offset, core_id, cmd_id, subnet_id):
        assert cmd_id is not None, "AKS-family targets must assign cmd_id manually"
        ...
        # get op struct
        op_clazz = self.op_class_dic[op_info.name]
    ...
    def decode_dma_cmds(
        ...
        *,
        ...
        while offset < len(reg_buf):
            ...
            res.append(cmd)
            if self.buf_is_end(reg_buf[offset:], cmd, self.dma_sys):
                break
        return res
    ...
    def decode_tiu_cmds(
        ...
        *,
        ...
        while offset < len(reg_buf):
            ...
            offset += cmd.reg.length // 8
            res.append(cmd)
            if self.buf_is_end(reg_buf[offset:], cmd, self.tiu_sys):
                break
        return res
    ...
    def decode_cdma_cmds(
        ...
        *,
        ...
        while offset < len(reg_buf):
            ...
            if (
                isinstance(cmd.reg, self.cdma_sys)
                and cmd.reg.cmd_special_function == 0
            ):
                break
            if self.buf_is_end(reg_buf[offset:], cmd, self.cdma_sys):
                break
        return res
