...
| 注意力变体                                  | 对训练的影响                | 对推理的影响                                                                            |
| -------------------------------------- | --------------------- | --------------------------------------------------------------------------------- |
| **MHA**                                | 标准基准；QKV 全量计算，显存峰值高   | KV Cache 占用 = $2 \times n\_{\text{heads}} \times d\_{\text{head}} \times L$，长序列瓶颈 |
| **GQA**                                | KV 头数减少，TP 切分策略不同     | KV Cache 显著减小（n\_kv\_heads/n\_heads 倍），Llama-2/3 标配                               |
...
┌──────────────────────────────────────────────┐
│  输入: (prompt, response\_A, response\_B)       │
│  人类标注: response\_A > response\_B            │
...
│  目标: $\max \mathbb{E}\[\text{reward} - \beta \cdot \text{KL}(\pi \parallel \pi\_{\text{ref}})]$      │
...
| **FlexAttention**               | 不同 Attention 变体需要不同 kernel | PyTorch 2.5+ 的可组合 Attention API，在 Python 层定义 score\_mod                    | PyTorch 官方              |
