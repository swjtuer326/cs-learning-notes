...
**根因**：Muon 优化器的 Newton-Schulz 正交化迭代中，中间矩阵的元素值可能超出 bf16 的表示范围（bf16 最大约 $3.39 \times 10^{38}$，但精度仅 7-bit 尾数），累积的舍入误差导致迭代发散。

**修复**：将 Newton-Schulz 迭代的**状态和中间量**显式转换为 float16（而非 bf16）。float16 的 10-bit 尾数在高精度迭代中远比 bf16 的 7-bit 尾数稳定。其余训练管线保持混合精度不变。此改动后 loss spike 完全消失。
...
ERNIE 5.0 的弹性训练在紧凑变体中仅使用 **53.7% 的激活参数**和 **35.8% 的总参数**，性能几乎不变。这意味着从一个 2.4T 参数的超网（supernet）中，可以零成本提取出约 800B 参数的 ERNIE 5.1 模型，预训练成本降低约 94%。
