...
**为什么 CPU 不阻塞？** 关键在 ring buffer 是异步队列——CPU 写完 descriptor 即返回，不等 GPU 取走。GPU FE 按提交顺序取 descriptor 并执行，保证同 stream 内 Kernel 串行。Launch queue 深度受 KMD channel 容量限制——具体上限由 driver 与 GPU 代际决定，NVIDIA 未在公开文档中给出固定数字，实践中可容纳数百至上千个 pending 任务，只要队列未满，CPU 可连续提交。
...
%%{init: {"theme": "base", "themeVariables": {"primaryColor": "#f8fafc", "primaryTextColor": "#1e293b", "primaryBorderColor": "#475569", "lineColor": "#64748b", "secondaryColor": "#f1f5f9", "secondaryBorderColor": "#94a3b8", "tertiaryColor": "#f8fafc", "fontFamily": "\"trebuchet ms\", verdana, arial, sans-serif"}}}%%
gantt
    title CPU-GPU 执行时间线（同步 API 的浪费）
    dateFormat X
    axisFormat %s
    
    section CPU (Host)
    cudaMalloc (CPU 工作)           :0, 1
    cudaMemcpy H2D (CPU 阻塞)       :crit, 1, 3
    启动 Kernel (瞬间返回)          :milestone, 3, 3
    cudaMemcpy D2H (阻塞,等GPU完)   :crit, 3, 7
...
**如何读这张图**：
- 红条（crit）= CPU 被同步 API 阻塞，白条 = 硬件在执行工作
- CPU **1→3**：被 `cudaMemcpy(H2D)` 阻塞，等 GPU 传完 H2D
- CPU **3→7**：被 `cudaMemcpy(D2H)` 阻塞——调用瞬间就睡，但 GPU 在 **3→5** 还在跑 Kernel，**5→7** 才做 D2H。CPU 多等了 **3→5** 这一整段

**问题**：GPU 跑 Kernel(3→5)时 CPU 已经在阻塞空等，两个资源无法并发。

**答案**：使用异步传输和 Stream，让 CPU 在 GPU 跑 Kernel 时做别的工作（见第 3 节）。
...
- `cudaMalloc` / `cudaFree`:释放内存会等待同一上下文中所有未完成 Kernel 完成(参考 CUDA C Programming Guide §3.2.5 "Asynchronous Concurrent Execution" 中的 "Implicit Synchronization")。原因:Driver 必须确保没有 Kernel 还在使用待释放的内存
...
3. **跨 stream 同步**：依赖图跨所有 stream——Kernel A 在 stream 0、Kernel B 在 stream 1 都引用 ptr → `cudaFree(ptr)` 必须等 A 和 B 都完成，即使 stream 0/1 之间无数据依赖（参考 CUDA C Programming Guide §3.2.5 "Asynchronous Concurrent Execution" 中的 "Implicit Synchronization" 段）。
...
> 建议参考 CUDA C Programming Guide §3.2.5 "Asynchronous Concurrent Execution" 中的 "Implicit Synchronization" 段了解细节。
...
**Stream 的硬件映射**（参考 CUDA C Programming Guide §3.2.5 "Asynchronous Concurrent Execution" 中的 "Streams" 段）：
...
**三种同步策略**（`cudaSetDeviceFlags`，参考 CUDA C Programming Guide §3.2.5 "Asynchronous Concurrent Execution" 中关于 `cudaDeviceSchedule*` 标志的说明，以及 CUDA Runtime API `cudaSetDeviceFlags` 条目）：
...
**Warp 级 Scoreboarding**：SM 的 Warp Scheduler 每个 cycle 选 ready warp 分发到 SM。判定 ready 的依据是 Warp Scoreboard——若 warp 当前指令依赖前一条长延迟指令（如 global memory load、FP64 op）的结果，scoreboard 标记该 warp 为"等待中"，scheduler 选其他 warp 执行。这是 **延迟隐藏 (Latency Hiding)** 的硬件基础——单 warp 等 memory 时，SM 切换到其他 warp 继续工作（参考 CUDA C Programming Guide §7 "Hardware Implementation" 中的 "Hardware Multithreading" 段）。
...
1. **SM 数量 + 资源占用**：SM 上同时驻留的 Kernel 数受寄存器/共享内存等资源约束——A100 108 SM 在理论上可同时驻留数十个低 Occupancy Kernel，但单 Kernel 大量消耗资源时只能并发 1-2 个
2. **寄存器/共享内存占用**：高 Occupancy Kernel 已占满 SM 资源 → 无法再并发其他 Kernel
3. **低 Occupancy Kernel 才适合并发**：每 Kernel 用 1/4 SM 资源时，理论上 4 个 Kernel 可并发驻留同一 SM
...
**为什么单 stream 内 Kernel 必须串行？** 即使资源充足，同 stream 内 Kernel 也按 FIFO 顺序执行——这是 stream 的"可预测性保证"。若允许同 stream 内 Kernel 并发，开发者必须给每个 Kernel 加显式同步，破坏了 stream 抽象的简洁性（参考 CUDA C Programming Guide §3.2.5 "Asynchronous Concurrent Execution" 中的 "Streams" 段）。
...
**注意事项**：
- Event 可在任意 Stream 中记录（详见 §4.5.2 跨 Stream 计时）；用于 `cudaEventElapsedTime` 的两个 Event 也不要求在同 Stream 中
- `cudaEventElapsedTime` 必须在两个事件都完成后调用
- 计时精度约 0.5 微秒（CUDA C Programming Guide "Asynchronous Concurrent Execution → Elapsed Time" 段给出规格）
...
**5 步硬件时间戳路径**（基于公开的 GPU command processor 架构推断；具体细节 NVIDIA 未在白皮书中完整公开，以下为通用 GPU command buffer 处理模式的合理推断）：
...
**忙等的代价**：`cudaEventQuery` 在内部仍是一次用户态→libcuda 的调用 + 一次内存读取（读 event 的 pinned host memory），单次开销很小但不可忽略。若循环上百万次，CPU 会浪费大量时间在轮询上。**长等待场景**应改用 `cudaEventSynchronize` + `cudaEventBlockingSync` 标志（见 §4.5.5）。

#### 4.5.4 计时精度来源

CUDA C Programming Guide "Asynchronous Concurrent Execution → Elapsed Time" 段给出 Event 计时的精度规格：

> The resolution of the timer is approximately 0.5 microseconds.

**0.5 μs 精度的来源**：并非 GPU 时钟本身精度不够——GPU 的内部时钟计数器在 GHz 量级（每 tick 约 1 ns 级别），远高于 0.5 μs。精度限制的真正来源是 **marker 命令的处理路径延迟**：FE 从 ring buffer 取到 marker 命令、读时钟计数器、把 tick 值写回 pinned host memory 这一连串动作要经过若干时钟周期与跨芯片路径，叠加 marker 排在 ring buffer FIFO 中的等待时间，最终导致"GPU 写入时间戳的时刻"与"事件逻辑触发的时刻"之间有约 0.5 μs 的抖动。这意味着：

- 测量**毫秒级** Kernel：精度足够（误差 < 0.1%）
- 测量**微秒级** Kernel：误差可能达 10%-50%，需重复执行多次取平均
- 测量**纳秒级**操作（如单条指令）：**不能用 Event**，应改用 `clock64()` 内置函数在 Kernel 内部测量

**为什么不能更精确？** 因为 Event marker 是按 stream FIFO 顺序处理的——FE 在 marker 之前若有任何任务未完成，marker 就会等待。marker 处理路径本身（取命令→读时钟→写回）的延迟构成精度下限。

**消除精度限制的替代方案**：

| 方法 | 适用场景 | 精度 | 限制 |
|------|----------|------|------|
| `cudaEventElapsedTime` | 跨 Kernel、Kernel+Memcpy | ~0.5 μs | 受 marker 处理路径延迟限制 |
| Kernel 内 `clock64()` | Kernel 内代码段 | 1 cycle（~ns 量级） | 仅 Kernel 内可见，不能跨 Kernel |
| `cudaProfilerStart/Stop` + nsight | 全程序热点分析 | ns 量级 | 离线分析，非运行时查询 |
...
/* 用途：cudaEventSynchronize(e_block) 时，CPU 线程被 OS 挂起，
   等 GPU 通过中断唤醒而非忙等
   收益：长等待时 CPU 不占 cycles；代价：唤醒延迟由 OS 调度决定，
   通常在微秒级，比忙等慢一个数量级 */
```
...
**为什么 `cudaEventDisableTiming` 与计时分开设计？** 因为计时并非免费——每个 marker 命令需读 GPU 时钟、写 pinned memory，路径上每个动作都消耗若干时钟周期。在不需要 `cudaEventElapsedTime` 的纯同步场景（如 stream 间依赖图），关闭计时能让 marker 路径更短，在高频提交场景下能累积成可观节省。
...
- [CUDA C Programming Guide §3.2.5 Asynchronous Concurrent Execution](https://docs.nvidia.com/cuda/cuda-c-programming-guide/index.html#asynchronous-concurrent-execution) — 参考了 Stream 和 Event 的语义、Implicit Synchronization、Concurrency、cudaDeviceSchedule* 同步策略（章节号以 CUDA 12 Programming Guide 为准；CUDA 13 起重编为 §6.2.8）
- [CUDA C Programming Guide "Elapsed Time" 段](https://docs.nvidia.com/cuda/cuda-c-programming-guide/index.html#elapsed-time) — 参考了 Event 计时 ~0.5 μs 精度规格、`cudaEventElapsedTime` 语义
- [CUDA C Programming Guide §7 Hardware Implementation](https://docs.nvidia.com/cuda/cuda-c-programming-guide/index.html#hardware-implementation) — 参考了 SIMT 架构、Hardware Multithreading、Warp Scheduler 与延迟隐藏机制
- [CUDA C Programming Guide Cooperative Groups](https://docs.nvidia.com/cuda/cuda-c-programming-guide/index.html#cooperative-groups) — 参考了协作组的使用
- [CUDA Runtime API Reference](https://docs.nvidia.com/cuda/cuda-runtime-api/) — 参考了 cudaEventCreateWithFlags / cudaEventDisableTiming / cudaEventBlockingSync / cudaSetDeviceFlags 标志语义
- [NVIDIA V100 Architecture Whitepaper](https://images.nvidia.com/content/volta-architecture/pdf/volta-architecture-whitepaper.pdf) — 参考了 Streaming Multiprocessor (Warp Scheduler / Scoreboard) 设计
